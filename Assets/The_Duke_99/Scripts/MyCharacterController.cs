...
public class MyCharacterController : MonoBehaviour
{
    ...
    [Range(0, 1)]
    [Tooltip(
        "The resilient input while character airborne. [0] player can't not control while airborne, [1] player freely move while airborne"
    )]
    [ConditionalField(nameof(MoveMethod), false, MovementMethod.AddForce)]
    ...
    [Tooltip(
        "FALSE: Disable movement during the dash duration, TRUE: cancel dash by movement input press"
    )]
    ...
    {
        set => Physics.gravity = Vector3.down * value * (value < 0 ? -1 : 1);
    }
    ...
    public float MoveVelocity { get; private set; } = 0;
    ...
    private void FixedUpdate()
    {
        ...
        {
            int round = 100;
            m_Debug.PhysicsSpeed = (float)(
                System.Math.Truncate(new Vector2(Rb.velocity.x, Rb.velocity.z).magnitude * round)
                / round
            );
            m_Debug.FlatPhysicsSpeed = (float)(
                System.Math.Truncate(Rb.velocity.magnitude * round) / round
            );
    ...
    bool GroundCheck()
    {
        ...
        {
            if (m_player == null)
                Debug.LogWarning("Missing player reference");
            return false;
        }
        ...
        {
            // Skip empty info(s)
            if (info.Value == null || info.Value.Count == 0)
                continue;
    ...
    void StoreCollisionData(Collision collision, bool isEntered = true)
    {
        ...
        if (!collisionInfos.ContainsKey(collision.gameObject))
        {
            collisionInfos.Add(collision.gameObject, new());
        }
    ...
    public void Move(Vector2 Input, CameraPerspective Perspective, float speed)
    {
        ...
        {
            m_targetVelocity =
                m_moveSpeed * DirectionWithCamera(new(MovementInput.x, 0, MovementInput.y));
        }
    }
    ...
    {
        if (m_perspective != CameraPerspective.ThirdPeron || !m_enableRotation)
            return;
        ...
        if (MovementInput.magnitude >= .1f)
        {
            Vector3 moveDir = MoveMethod switch
            {
                ...
                MovementMethod.AddForce => DirectionWithCamera(
                    new(MovementInput.x, 0, MovementInput.y)
                ),
                _ => Vector3.zero,
            };

            if (
                Vector3.Dot(moveDir, DirectionWithCamera(new(MovementInput.x, 0, MovementInput.y)))
                >= .9f
            )
            {
                float targetAngle =
    ...
    void MoveByVelocity()
    {
        ...
        {
            Debug.LogWarning(
                "Missing components in MyCharacterController in " + name + " GameObject"
            );
            return;
        }
        ...
        m_smoothVelocity = Vector3.SmoothDamp(
            m_smoothVelocity,
            m_targetVelocity,
            ref ref_moveVelocity,
            !EnabledGroundCheck ? SmoothGroundMovement
                : IsGrounded ? SmoothGroundMovement
                : SmoothAirMovement
        );
    ...
    void MoveByPosition()
    {
        ...
        {
            Debug.LogWarning(
                "Missing Rigidbody in MyCharacterController in " + name + " GameObject"
            );
            return;
        }
        ...
        MoveVelocity = m_smoothVelocity.magnitude;
    }
    ...
    public void LookAt(Vector3 targetToLook, float smoothRotation)
    {
        ...
        transform.eulerAngles =
            Vector3.up
            * Mathf.LerpAngle(transform.eulerAngles.y, angle, smoothRotation * Time.deltaTime);
    }
    ...
    public void Jump(float strength, JumpMethod method = JumpMethod.ChangeVelocity)
    {
        ...
        {
            Debug.LogWarning(
                "Missing Rigidbody component in My Character Controller in " + name + " GameObject"
            );
            return;
        }
    ...
    Vector3 DirectionWithCamera(Vector3 Input)
    {
        float targetRotation =
            Mathf.Atan2(Input.x, Input.z) * Mathf.Rad2Deg + MainCamera.transform.eulerAngles.y;
        Vector3 result =
            Quaternion.Euler(0, targetRotation, 0)
            * Vector3.forward
            * (Input.magnitude >= .1f ? 1 : 0);
    ...
    IEnumerator Dashing(Vector3 direction)
    {
        ...
        while (timer < DashDuration)
        {
            ...
            Vector3 dashDir =
                direction == Vector3.zero ? transform.forward.normalized : direction.normalized;
    ...
    private void OnDrawGizmos()
    {
        ...
        if (m_Debug.MovementDebug && Application.isPlaying)
        {
            ...
            Gizmos.DrawLine(
                ...
                transform.position + Vector3.up + transform.forward.normalized * 1
            );
        }
    }
}
