...
namespace MyBox.Internal
{
    ...
    public class MyCursorPropertyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.EndChangeCheck())
                hotspot.vector2Value = Vector2.zero;
    ...
    public class HotspotEditorPopup : PopupWindowContent
    {
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            GUILayout.Label(
                _texture,
                GUILayout.Width(_texture.width),
                GUILayout.Height(_texture.height)
            );
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (
                    GUILayout.Button(
                        MyGUI.Characters.Cross,
                        EditorStyles.toolbarButton,
                        GUILayout.Width(40)
                    )
                )
                    _hotspotProperty.vector2Value = new Vector2(
                        _texture.width / 2f,
                        _texture.height / 2f
                    );
                if (GUILayout.Button("→", EditorStyles.toolbarButton, GUILayout.Width(40)))
